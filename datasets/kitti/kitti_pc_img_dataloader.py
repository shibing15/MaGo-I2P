...
class kitti_pc_img_dataset(data.Dataset):
    ...
    def make_kitti_dataset(self, root_path, mode):
        ...
        np_folder = 'stride4-acc50-voxel0.1'

        skip_start_end = 0
        for seq in seq_list:
            img2_folder = os.path.join(
                root_path, 'data_odometry_color_npy' , 'sequences', '%02d' % seq, 'image_2')
            img3_folder = os.path.join(
                root_path, 'data_odometry_color_npy' , 'sequences', '%02d' % seq, 'image_3')
            depth_2_folder = os.path.join(
                root_path, 'data_odometry_depth_npy' , 'sequences', '%02d' % seq, 'image_2')
            depth_3_folder = os.path.join(
                root_path, 'data_odometry_depth_npy' , 'sequences', '%02d' % seq, 'image_3')
            # pc_folder = os.path.join(
            #     root_path, 'data_odometry_velodyne' , 'sequences', '%02d' % seq, np_folder)
            pc_folder = os.path.join(
                root_path, 'data_odometry_velodyne', 'sequences', '%02d' % seq, np_folder)
            ...
            for i in range(skip_start_end, sample_num - skip_start_end):
                dataset.append((img2_folder, pc_folder,
                                K2_folder, seq, i, 'P2', sample_num, depth_2_folder))
                dataset.append((img3_folder, pc_folder,
                                K3_folder, seq, i, 'P3', sample_num, depth_3_folder))
        return dataset
    ...
    def rotation_maxtrix2quaternion(self, R):
    ...
    def generate_random_transform(self):
        ...
        P_inv = np.linalg.inv(P_random)
        quaternion = self.rotation_maxtrix2quaternion(P_inv[0:3, 0:3])
        P_quat_random = np.concatenate((P_inv[0:3, 3], quaternion))
        # print('t',t)
        # print('angles',angles)

        return P_random, P_quat_random

    def __len__(self):
        return len(self.dataset)

    def __getitem__(self, index):
        img_folder, pc_folder, K_folder, seq, seq_i, key, _, depth_folder = self.dataset[index]
        img = np.load(os.path.join(img_folder, '%06d.npy' % seq_i))
        img_vis = img
        data = np.load(os.path.join(pc_folder, '%06d.npy' % seq_i))
        depth_map = np.load(os.path.join(depth_folder, '%06d_depth.npy' % seq_i))
        intensity = data[3:4, :]
        sn = data[4:, :]
        pc = data[0:3, :]
        ...
        depth_map = cv2.resize(depth_map,
                               (int(round(depth_map.shape[1] * 0.5)),
                                int(round(depth_map.shape[0] * 0.5))),
                            interpolation=cv2.INTER_LINEAR)
        ...
        depth_img = depth_img[img_crop_dy:img_crop_dy + self.img_H,
              img_crop_dx:img_crop_dx + self.img_W, :]
        ...
        #coo_matrix((data,(row, col)),shape)的方式产生矩阵
        ...
        P, P_quat = self.generate_random_transform()
        ...
        return {'img': torch.from_numpy(img.astype(np.float32) / 255.).permute(2, 0, 1).contiguous(),
                ...
                # 'P_quat': torch.from_numpy(P_quat.astype(np.float32)),
...
if __name__ == '__main__':
    ...
    dataset = kitti_pc_img_dataset('/media/ai-i-sunyunda/data/data/KITTI_for_DEEPI2P/', 'val', 20480 )
    trainloader=torch.utils.data.DataLoader(dataset,batch_size=4,shuffle=True,drop_last=True,num_workers=0)
