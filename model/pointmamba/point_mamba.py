...
class PointMamba_Pointwise(nn.Module):
    def __init__(self, config, **kwargs):
        super(PointMamba_Pointwise, self).__init__()
        self.config = config
        ...
        self.pc_global_layer=nn.Sequential(nn.Conv1d(1152,1152,1,bias=False),nn.BatchNorm1d(1152),nn.ReLU(),nn.Conv1d(1152,1152,1,bias=False),nn.BatchNorm1d(1152),nn.ReLU(),nn.Conv1d(1152,512,1,bias=False))
    ...
    def forward(self, pts):
        B, C, N = pts.shape
        pts = pts.permute(0,2,1).contiguous()  # B N 3
        ...
        global_pc_feat = self.pc_global_layer(x_max.unsqueeze(2))
        ...
        # x = x.permute(0, 2, 1)
        return global_pc_feat.squeeze(2), x

class Point_wised_head(nn.Module):
    def __init__(self, input_channel, output_channel):
        ...
        self.convs2 = nn.Conv1d(512, 256, 1)
        # self.convs3 = nn.Conv1d(256, self.cls_dim, 1)
        self.convs3 = nn.Conv1d(256, output_channel, 1)
        self.bns1 = nn.BatchNorm1d(512)
        self.bns2 = nn.BatchNorm1d(256)
        self.relu = nn.ReLU()
...
class PointMamba_point_cla(nn.Module):
    def __init__(self, config, **kwargs):
        super(PointMamba_Pointwise, self).__init__()
        self.config = config
        ...
        self.point_wise_head = Point_wised_head(3328,128)

        # img_W = int(self.config.img_W / self.config.img_fine_resolution_scale)
        # img_H = int(self.config.img_H / self.config.img_fine_resolution_scale)
        # self.point_classifier = Point_classifier(3328, img_W, img_H)
    ...
    def forward(self, pts):
        ...
        point_wise_features = self.point_wise_head(x)

        # point_cla_score = self.point_classifier(x)
        # x = x.permute(0, 2, 1)
        return global_pc_feat.squeeze(2), point_wise_features
